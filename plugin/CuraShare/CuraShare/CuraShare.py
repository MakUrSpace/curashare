...
class CuraShare(Extension, QObject,):
    def __init__(self, parent = None) -> None:
        ...
        self._networkManager.finished.connect(self.on_request_succeed)
        ...
        self.setMenuName(catalog.i18nc("@item:inmenu", "CuraShare"))
    ...
    def on_request_succeed(self, reply: QNetworkReply):
        statusCode = None
        try:
            statusCode = reply.attribute(QNetworkRequest.Attribute.HttpStatusCodeAttribute)
            assert statusCode == 200
        except:
            Logger.logException("e", f"Failed to post network request: ({statusCode}) {reply}")
    ...
    def exportData(self) -> None:
        ...
        try:
            with open(file_name, 'r', newline='') as csv_file:
                dataString = csv_file.read()
            profileId = str(uuid4())
            url = QUrl(f"")

            req = QNetworkRequest(url)
            self._networkManager.post(req, dataString.encode())
        except:
            Logger.logException("e", "Failed to upload profile")
            return

        Message().hide()
        Message("Exported data for profile %s" % P_Name, title = "Import Export CSV Profiles Tools").show()
